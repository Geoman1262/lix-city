...
function setFloaters(hidden){['v25btn','lix24open','v26ArcadeBtn'].forEach(id=>{const e=document.getElementById(id);if(e)e.style.display=hidden?'none':''})}
function closeCommand(){const e=panel();if(e)e.classList.remove('open');setFloaters(false)}
...
el.innerHTML=`<div class="v25top"><b>⚡ LIX CITY COMMAND</b><button class="v25exit" data-x="close" aria-label="Exit Command">×</button></div><div class="v25stats"><span>🏠 <b>${p.level||1}</b><small>Level</small></span><span>🪙 <b>${p.coins||0}</b><small>Coins</small></span><span>⚡ <b>${p.energy||0}/${p.maxEnergy||10}</b><small>Energy</small></span><span>🔥 <b>${D.streak}</b><small>Streak</small></span></div>
