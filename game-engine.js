...
(function(){
  ...
  function finish(id,win,score=0){const s=load();if(!s[id])return;const session=s.session;if(session && session.id!==id)return;s[id].wins+=win?1:0;s[id].losses+=win?0:1;s[id].best=Math.max(Number(s[id].best)||0,Number(score)||0);s.lastGame=id;s.lastResult=win?'win':'loss';s.session=null;save(s);try{if(window.S){S.v57=S.v57||{};if(win)S.v57.totalWins=(Number(S.v57.totalWins)||0)+1;S.v57.lastGame=id;window.save?.()}}catch(e){}}
